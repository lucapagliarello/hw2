...
<html>
    ...
    <div class="rc">
        ...
        <div class="btn-home-container">
            <a href="{{ url('home') }}" class="btn-home">Torna alla Home</a>
        </div>

        <form method="POST" action="{{ url('/aggiungi') }}">
            @csrf

            <label>Qualità del servizio:
                <input type="range" name="qualita" min="1" max="5" required value="{{ old('qualita') }}">
            </label><br>

            <label>Spedizione:
                <input type="range" name="spedizione" min="1" max="5" required value="{{ old('spedizione') }}">
            </label><br>

            <label>Assistenza clienti:
                <input type="range" name="assistenza" min="1" max="5" required value="{{ old('assistenza') }}">
            </label><br>

            <label>Commento:
                <textarea name="commento" rows="5" cols="40" required>{{ old('commento') }}</textarea>
            </label><br>
        ...
        <div id="recensioni-container">
            @forelse ($recensioni as $recensione)
                <div class="recensione">
                    <h4>Utente: {{ $recensione->username }}</h4>
                    <p>
                        Qualità: {{ $recensione->qualita }} |
                        Spedizione: {{ $recensione->spedizione }} |
                        Assistenza: {{ $recensione->assistenza }}
                    </p>
                    <p>Commento: {{ $recensione->commento }}</p>
                    <p><small>Data: {{ $recensione->data->format('d/m/Y H:i') }}</small></p>
                </div>
            @empty
